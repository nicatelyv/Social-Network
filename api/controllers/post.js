...
export const getPosts = (req, res) => {
    ...
    if (!token) return res.status(401).json("Daxil olmamısınız!");


    jwt.verify(token, "secretkey", (err, userInfo) => {
        if (err) return res.status(403).json("Token etibarlı deyil!");
...
export const addPost = (req, res) => {
    ...
    jwt.verify(token, "secretkey", (err, userInfo) => {
        ...
        db.query(q, [values], (err, data) => {
            if (err) return res.status(500).json(err);
            return res.status(200).json("Post yaradıldı!");
        });
    });
};
export const deletePost = (req, res) => {
    ...
    jwt.verify(token, "secretkey", (err, userInfo) => {
        ...
        db.query(q, [req.params.id, userInfo.id], (err, data) => {
            if (err) return res.status(500).json(err);
            if (data.affectedRows > 0) return res.status(200).json("Post silindi!");
